...
public:
    ...
    ~Blob()
    {
        ...
        if (d_ptr_ != nullptr)
            cudaFree(d_ptr_);
        if (is_tensor_)
            cudnnDestroyTensorDescriptor(tensor_desc_); // 销毁张量描述符
    }

    // reset the current blob with the new size information
    void reset(int n = 1, int c = 1, int h = 1, int w = 1) // 为数据重新分配张量
    {
    ...
    int size() { return c_ * h_ * w_; } // 一个批次的大小

    // returns number of total elements in blob including batch
    int len() { return n_ * c_ * h_ * w_; } // 总长度
    ...
    cudnnTensorDescriptor_t tensor()
    {
        ...
        //                            CUDNN_TENSOR_NCHW, // 张量的存储格式
        //                            CUDNN_DATA_FLOAT,  // 张量的数据类型
        //                            n_, c_, h_, w_);
        cudnnSetTensor4dDescriptor(tensor_desc_,
                                   CUDNN_TENSOR_NCHW,         // 张量的存储格式
                                   getCudnnDataType<ftype>(), // 张量的数据类型
                                   n_, c_, h_, w_);
        ...
        if (view_param)
        {
            ...
            for (int n = 0; n < num_batch; n++)
            {
                ...
                while (count < size() && print_line_count < max_print_line)
                {
                    ...
                    {
                        std::cout << float(h_ptr_[size() * n + count + offset]) << "\t";
                        count++;
                    }
                    std::cout << std::endl;
                    print_line_count++;
    ...
    int file_read(std::string filename)
    {
        ...
        file.read((char *)h_ptr_, sizeof(ftype) * this->len());
    ...
    int file_write(std::string filename)
    {
        ...
        file.write((char *)this->to(host), sizeof(ftype) * this->len());
        file.close();
